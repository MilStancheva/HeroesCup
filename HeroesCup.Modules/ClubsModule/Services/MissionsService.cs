...
namespace ClubsModule.Services
{
    public class MissionsService : IMissionsService
    {
        ...
        public TimeheroesMissionEditModel CreateTimeheroesMissionEditModelAsync()
        {
           var model = new TimeheroesMissionEditModel()
            {
                Mission = new Mission(),
            };

            model.Mission.Type = MissionType.TimeheroesMission;
            return model;
        }

        public async Task<TimeheroesMissionEditModel> GetTimeheroesMissionEditModelByIdAsync(Guid id)
        {
            Mission mission = null;
            mission = await this.dbContext.Missions
                    .Where(m => m.Type == MissionType.TimeheroesMission)
            ...
            var model = CreateTimeheroesMissionEditModelAsync();
            model.Mission = mission;
        ...
        public async Task<Guid> SaveTimeheroesMissionEditModelAsync(TimeheroesMissionEditModel model)
        {
            var mission = await this.dbContext.Missions
                  .Include(c => c.MissionImages)
                  .ThenInclude(m => m.Image)
                  .FirstOrDefaultAsync(m => m.Id == model.Mission.Id);
            ...
            mission.Content = model.Mission.Content;
            mission.TimeheroesUrl = model.Mission.TimeheroesUrl;
            mission.Type = MissionType.TimeheroesMission;
